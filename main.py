...
class TradeManager:
    ...
    def check_all_positions(self, snapshot) -> List[Dict]:
        ...
        with self._lock:
            for signal_id, pos in list(self.positions.items()):
                ...
                # ===== P4.32: SOFT PROFIT LOCK (after TP1, gradual SL based on MFE) =====
                self._update_profit_lock(pos, current_price)
    ...
    def _update_trailing_stop(self, pos: OpenPosition, current_price: float):
        """
        P3: Exit State Machine based trailing stop.
        
        SEEK → CAPTURE → DEFEND → HARVEST
        
        Replaces: Fixed profit trigger (removed)
        New: State-based trailing that lets winners run longer
        """
        ...
        exit_state = _compute_exit_state(pos, current_price, atr_pct)
    ...
    def _update_profit_lock(self, pos: OpenPosition, current_price: float):
        """
        P4.32 — Soft profit lock: setelah TP1 hit, SL naik GRADUAL berdasarkan MFE.
        JANGAN: if tp1: sl=entry (bunuh runner)

        FIX (exit-efficiency): formula lama `lock_ratio = min(0.3, mfe_pct/10)`
        cuma ngunci ~10% dari MFE yang dicapai, DIJEPIT flat di 0.3% absolut —
        makin tinggi MFE, makin KECIL proporsi yang ke-protect (di MFE 10%,
        cuma 3% dari gain yang ke-lock). Itu kenapa posisi yang sempat profit
        besar (MFE +2.63%) bisa exit cuma +0.07% (exit_eff 3%) pas reversal —
        SL-nya cuma di-lock +0.263% dari entry, nyisa 90% gain "telanjang".

        Formula baru: lock_fraction (porsi dari MFE yang dikunci) NAIK
        seiring MFE makin besar — kecil di awal (15%, biar gak bunuh runner
        karena noise kecil), naik bertahap, sampai cap 70% di MFE besar
        (biar mayoritas gain udah ke-protect, runner masih punya 30% ruang
        napas buat lanjut jalan tanpa langsung kena stop).

        FIX 2 (ATR floor + regime-adaptive, P4.57): trade ALT (MFE +0.14%,
        exit 3 menit) menunjukkan masalah baru — lock_fraction=0.15 * MFE
        kecil (0.14%) menghasilkan lock_distance MIKROSKOPIS (0.021% dari
        entry), SL jadi nyaris nempel entry walau volatilitas asli coin itu
        jauh lebih besar dari 0.14%. Noise harga normal pun cukup buat kena
        SL dalam hitungan menit. Fix: lock_pct di-floor ke proporsi ATR coin
        (0.2 * entry_atr_pct), supaya SL gak pernah lebih ketat dari ~20%
        rentang volatilitas normalnya — apapun MFE-nya. Plus lock_fraction
        disesuaikan regime: TRENDING lebih longgar (biar runner ikut tren),
        RANGING lebih ketat (lock profit lebih cepat sebelum reversal).
        regime="UNKNOWN" (candle historis <30, jarang terjadi) pakai
        lock_fraction dasar yang sama seperti sebelumnya — tidak diubah.

        NOTE: OpenPosition tidak punya field `mfe` — MFE dihitung inline dari
        pos.highest/pos.lowest (sama seperti di _close_remaining), bukan pos.mfe.
        """
        if not pos.tp1.is_hit or pos.status != "OPEN":
            return

        if pos.direction == "LONG":
            mfe_pct = (pos.highest - pos.entry) / pos.entry * 100
        else:
            mfe_pct = (pos.entry - pos.lowest) / pos.entry * 100
        ...
        if mfe_pct < 1.0:
            ...
            lock_fraction = 0.15 + (mfe_pct - 1.0) / 2.0 * 0.35  # 0.15 → 0.50
        else:
            lock_fraction = min(0.70, 0.50 + (mfe_pct - 3.0) * 0.05)

        # Regime-adaptive: trending = lebih longgar (biar runner ikut tren),
        # ranging = lebih ketat (lock cepat sebelum reversal). UNKNOWN/lainnya
        # tidak diubah dari formula dasar di atas.
        if pos.regime in ("TRENDING_UP", "TRENDING_DOWN"):
            lock_fraction *= 0.7
        elif pos.regime == "RANGING":
            lock_fraction *= 1.2

        lock_pct = mfe_pct * lock_fraction  # langsung dalam persen, bukan rasio terpisah

        # ATR floor: lock_pct gak boleh lebih kecil dari proporsi volatilitas
        # struktural coin ini, supaya SL gak nempel entry cuma karena MFE
        # awal kecil (kasus trade ALT: MFE 0.14% -> lock_pct 0.021% tanpa floor).
        if pos.entry_atr_pct > 0:
            min_lock_pct = 0.2 * pos.entry_atr_pct
            lock_pct = max(lock_pct, min_lock_pct)

        lock_distance = pos.entry * (lock_pct / 100)

        if pos.direction == "LONG":
            new_sl = max(pos.sl, pos.entry + lock_distance)
            if new_sl > pos.sl:
                pos.sl = new_sl
                logger.info(f"🔒 PROFIT LOCK {pos.coin}: SL → {new_sl:.4f} (lock_pct={lock_pct:.3f}%, mfe={mfe_pct:.2f}%, fraction={lock_fraction:.2f}, regime={pos.regime})")
        else:
            new_sl = min(pos.sl, pos.entry - lock_distance)
            if new_sl < pos.sl:
                pos.sl = new_sl
                logger.info(f"🔒 PROFIT LOCK {pos.coin}: SL → {new_sl:.4f} (lock_pct={lock_pct:.3f}%, mfe={mfe_pct:.2f}%, fraction={lock_fraction:.2f}, regime={pos.regime})")
    ...
    try:
        ...
        if hypothesis_validated is not None:
            c.execute('''UPDATE signals SET evaluated=1, outcome=?, pnl=?, exit_price=?, exit_time=?, 
                         ...
                         feedback_early=?, feedback_shape=?, feedback_mature=?, leverage=? WHERE signal_id=?''',
                      (outcome, pnl, exit_price, int(time.time()), mfe, mae,
                       1 if hypothesis_validated else 0,
                       exit_eff, data_source, regime, cache_age,
                       feedback_early, feedback_shape, feedback_mature, leverage, signal_id))
        else:
            c.execute('''UPDATE signals SET evaluated=1, outcome=?, pnl=?, exit_price=?, exit_time=?, 
                         mfe=?, mae=?,
                         exit_eff=?, source=?, regime=?, cache_age=?,
...
def get_context_snapshot(coin: str = "BTC") -> ContextSnapshot:
    ...
    threading.Thread(target=log_context, args=(ctx, coin), daemon=True).start()
    return ctx
...
def compute_suggested_leverage(coin: str, entry: float, sl: float, position_size_mult: float,
                              conviction: float = 50.0, market_regime: str = "UNKNOWN") -> Dict[str, Any]:
    ...
    if risk_pct <= 0:
        return {"suggested": 1.0, "native_cap": native_cap, "risk_pct": 0.0}
    ...
    # ===== VOLATILITY MULTIPLIER (dari ATR pct) =====
    # Kita coba ambil ATR dari data yg ada
    try:
    ...
    # ===== COMBINED MULTIPLIER =====
    combined_mult = conviction_mult * regime_mult * vol_mult
        ...
        if USER_ID and not PAPER_MODE and decision_type == "EXECUTE":
            try:
                ...
                if not verified:
                    ...
                    _lev_info = compute_suggested_leverage(coin, mark, confidence_data['sl'], position_size_mult)
...
""")
    ...
    for i, coin in enumerate(candidates):
        ...
        # Check 3: Global API cooldown (P4.53 SOFT COOLDOWN — non-blocking)
        # Cooldown cuma boleh matiin LIVE fetch, BUKAN observability.
        # get_candles() sendiri sudah punya internal guard (can_call_api()
        # check di dalamnya) jadi force_live di sini cuma soal "boleh nyoba
        # live atau cache-only" — kalau live gagal karena cooldown/429,
        # get_candles akan return [] dan kita fallback ke cache di bawah.
        api_ok = can_call_api()
        ...
        # P4.53 GUARDRAIL: kalau cooldown aktif & cache udah terlalu tua, skip
        # (cegah keputusan dari data basi banget, bukan cegah observability).
        MAX_CACHE_AGE_THESIS = 300  # 5 menit
        if not force_live and cache_age > MAX_CACHE_AGE_THESIS:
            logger.warning(f"   SKIP_{coin}_cache_too_old ({cache_age:.0f}s > {MAX_CACHE_AGE_THESIS}s)")
            inc_pipeline_counter("cache_too_old_skip")
            continue

        want_live = force_live and should_refresh_live("THESIS", cache_age, rank)
        if want_live:
        ...
        # ===== Observability: catat SETIAP percobaan scan, terlepas hasil =====
...
def _build_compact_alert(alert: dict) -> str:
    ...
    conv_line = (
        f"├─ 🧠 CONV: score={conv_score:.0f} | penalty=-{conv_penalty:.0f} | "
    ...
    mem_line = (
        f"├─ 🧬 MEM: outcome={mem_edge:+.0f} | cooldown_mult={mem_cooldown:.2f} | "
        f"edge={mem_edge:+.0f} | stability={mem_stab_str}\n"
    )
    # ===== END P4.50 =====
...
def get_recent_win_rate(limit: int = 20) -> float:
    """Get recent win rate from last N evaluated trades."""
    try:
        conn = db_connect()
        c = conn.cursor()
        c.execute('''SELECT
...
def send_close_alert_compact(trade: dict):
    """Compact CLOSE alert — 5 lines, includes TP level."""
    emoji = "🟢" if trade["pnl"] > 0 else "🔴"
