...
def build_candidate_pool(max_candidates: int = 12) -> List[str]:
    """Stage A: discovery murah (snapshot/OI/funding saja, TANPA candles).
    Output: ~8-12 kandidat untuk deep analysis di Stage B."""
    snapshot = get_snapshot()
    if not snapshot or not snapshot.mids:
        return []
    ...
    for coin in get_alpha_coins(snapshot, limit=4):
        if coin not in seen and len(candidates) < max_candidates:
            candidates.append(coin)
            seen.add(coin)

    for coin in get_oi_flow_coins(snapshot, limit=6):
        if coin not in seen and len(candidates) < max_candidates:
            candidates.append(coin)
            seen.add(coin)

    for coin in get_narrative_coins(snapshot, limit=3):
        if coin not in seen and len(candidates) < max_candidates:
            candidates.append(coin)
            seen.add(coin)

    if len(candidates) < 6:
        top_vol = sorted(snapshot.oi.items(), key=lambda x: x[1], reverse=True)[:10]
        for coin, _ in top_vol:
            if coin not in seen and len(candidates) < max_candidates:
                candidates.append(coin)
                seen.add(coin)
    ...
    logger.debug(f"🔍 DISCOVERY V11: {len(candidates)} candidates: {candidates}")
    return candidates


# ============================================================
# V11 — STAGE B: DEEP ANALYSIS (candles + L2, dengan API budget)
# ============================================================
