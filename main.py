...
def collect_all_events(coin: str, current_price: float, master: Dict) -> List[TradeEvent]:
    ...
    if STRUCTURE_COMPARE:
        try:
            ...
            # INFO (bukan debug) — supaya keliatan di log tanpa perlu set
            # LOG_LEVEL=DEBUG di env (yang bakal bikin log lain ikut berisik).
            logger.info(
                f"STRUCTURE_COMPARE {coin}: V1={len(v1_events)} events "
                f"[{', '.join(e.type for e in v1_events)}] | "
                f"V2={len(v2_events)} events [{', '.join(e.type for e in v2_events)}]"
            )
        except Exception as e:
