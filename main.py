...
def sanitize_maps_from_snapshot(snapshot) -> None:
    ...
    Buang coin yang gak ada / OI < 250k.
    Jalankan tiap cycle di state_engine — zero extra API call.

    Side effects:
    - Update _SECTOR_MAP ke coins yang live saja
    - Update _NARRATIVE_MAP_LIVE dari _SECTOR_MAP yang sudah bersih
    - Log diff kalau ada perubahan
    """
    ...
    for coin, sector in _SECTOR_MAP_BASE.items():
        if coin in live_coins and snapshot.oi.get(coin, 0) >= min_oi:
    ...
    # Invert sector map: sector → [coins]
    ...
    logger.info(
        f"🗺️ MAPS SANITIZED | live={len(new_sector)} removed={len(removed)} "
        f"sectors={len(active_sectors)} coins_mapped={total_mapped} "
        f"sectors={active_sectors}"
    )
    if removed:
        logger.debug(f"   Removed dead coins: {removed[:15]}")
...
def get_oi_roc(coin: str, window_minutes: int = 5) -> float:
    """Rate of change of OI.

    window_minutes=5   → realtime (observe_market, scoring)
    window_minutes=60  → discovery (build_candidate_pool) karena OI gerak
                         1-5% per JAM, window 5m selalu flat → gate skip semua.

    Fallback otomatis: kalau window_minutes data belum ada tapi ada data lebih
    pendek, pakai yang ada (warmup-friendly).
    """
    with _oi_lock:
        if coin not in _oi_history or len(_oi_history[coin]) < 2:
            return 0.0
    ...
    # Fallback: kalau window request kosong (data belum cukup panjang),
    # pakai semua history yang ada — lebih baik estimasi kasar dari 0.
    if not oi_vals:
        oi_vals = [v for _, v in hist]
        if len(oi_vals) < 2:
            return 0.0
    ...
    # Sample log 0.5% chance biar gak spam
    if random.random() < 0.005:
        logger.debug(
            f"ROC_{coin} w={window_minutes}m n={len(oi_vals)} "
            f"avg={oi_avg:.0f} cur={oi_current:.0f} roc={roc:+.2f}%"
        )
