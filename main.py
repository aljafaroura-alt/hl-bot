...
def start_worker_threads():
    """Start semua background threads, skip yang masih hidup."""
    workers = {
        "state":  scheduled_state_engine,
        "trigger": scheduled_trigger_engine,
        "shadow": scheduled_shadow_evaluation,
        "clean":  scheduled_cleanup,
    }
    for name, target in workers.items():
        t = WORKER_THREADS.get(name)
        if t is None or not t.is_alive():
            logger.warning(f"[Watchdog] Restarting thread: {name}")
            t = threading.Thread(target=target, daemon=True, name=name)
            t.start()
            WORKER_THREADS[name] = t

def thread_watchdog():
    """Pantau thread-thread utama, restart kalo mati."""
    while True:
        try:
...
if __name__ == "__main__":
    ...
    # Start semua thread pertama kali
    start_worker_threads()

    # Watchdog thread buat monitor & restart thread yg mati
    t_watchdog = threading.Thread(target=thread_watchdog, daemon=True, name="watchdog")
    t_watchdog.start()

    # Start bot polling dengan auto reconnect loop
    while True:
        try:
            logger.info("[Bot] Starting infinity_polling...")
            bot.infinity_polling(timeout=60, long_polling_timeout=60)
        except Exception as e:
            logger.error(f"[Bot] Polling crashed: {e}. Reconnecting in 10s...")
            time.sleep(10)
