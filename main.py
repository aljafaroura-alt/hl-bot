...
def execute_decision(coin: str, thesis_data: Dict, confidence_data: Dict,
    ...
    if total_open >= MAX_TOTAL_OPEN:
        ...
        log_skip(coin=coin, reason="inventory_full", stage="GATE", cache_age=cache_age, data_source=data_source)
        return None
    ...
    if coin_open >= MAX_COIN_OPEN:
        ...
        log_skip(coin=coin, reason="coin_limit", stage="GATE", cache_age=cache_age, data_source=data_source)
        return None
    ...
    if clarity.get("dominant_factor") == "strong_contradiction":
        ...
        update_fatigue_memory(event.type)
        log_skip(coin=coin, reason="strong_contradiction", stage="GATE", cache_age=cache_age, data_source=data_source)
        return None
    ...
    if clarity["decision_quality"] < 0.40:  # Only skip on extreme chaos
        ...
        update_fatigue_memory(event.type)
        log_skip(coin=coin, reason=f"chaos_{clarity['decision_quality']:.2f}", stage="GATE", cache_age=cache_age, data_source=data_source)
        return None
        ...
        with TRADE_MANAGER._lock:
            ...
            if total_open > 0:
                ...
                if btc_share > 0.60 and coin != "BTC":
                    ...
                    update_fatigue_memory(event.type)
                    log_skip(coin=coin, reason="btc_concentration", stage="GATE", cache_age=cache_age, data_source=data_source)
                    return None
    ...
    if not conviction_data["is_qualified"]:
        ...
        update_fatigue_memory(event.type)
        log_skip(coin=coin, reason=reason, stage="GATE", cache_age=cache_age, data_source=data_source)
        return None
    ...
    if not micro_confirmed:
        ...
        update_fatigue_memory(event.type)
        log_skip(coin=coin, reason="micro_structure_gate", stage="GATE", cache_age=cache_age, data_source=data_source)
        return None
...
def check_entry_alert_v10_phase1(coin: str, mark: float, master_candles: Dict,
    ...
    try:
        ...
        if _fast_total_open >= _fast_max_total_open:
            ...
            log_skip(coin=coin, reason=f"inventory_full_{_fast_total_open}/{_fast_max_total_open}", stage="GATE", cache_age=cache_age, data_source=data_source)
            return None
        ...
        try:
            ...
            log_skip(coin=coin, reason="context_error", stage="OBS", cache_age=cache_age, data_source=data_source)
            return None
        ...
        if not obs or obs.get("status") == "REJECT":
            ...
            inc_pipeline_counter("reject_obs")
            log_skip(coin=coin, reason=reason, stage="OBS", cache_age=cache_age, data_source=data_source)
            return None
        ...
        if not thesis_data or thesis_data.get("status") == "REJECT":
            ...
            log_skip(coin=coin, reason=reason, stage="THESIS", cache_age=cache_age, data_source=data_source)
            return None
        ...
        if not confidence_data or confidence_data.get("status") == "REJECT":
            ...
            log_skip(coin=coin, reason=reason, stage="CONF", cache_age=cache_age, data_source=data_source)
            return None
