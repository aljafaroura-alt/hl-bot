...
def build_candidate_pool_v12(max_candidates: int = 12) -> List[str]:
    ...
    try:
        ...
        for coin in list(snapshot.mids.keys()):
            ...
            # Progressive gate: regime kuat + OI persist → gate bisa turun ke 0.07
            # HANYA dipanggil setelah lolos gate awal 0.10 (bukan semua ~230 coin)
            _effective_gate = get_progressive_gate(coin, base_magnitude=0.10)
        ...
        if _sample_fail_count:
            logger.info(
                ...
                f"avg_magnitude={_avg_mag:.4f}(need>=0.10) "
