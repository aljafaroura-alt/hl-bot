...
def is_micro_bos_up(candles_5m: List[dict]) -> bool:
    ...
    try:
        recent_lows = []
        for i in range(max(2, len(candles_5m)-6), len(candles_5m)-1):
            try:
                l_i  = float(candles_5m[i]['l'])
                l_p1 = float(candles_5m[i-1]['l'])
                l_p2 = float(candles_5m[i-2]['l'])
                if l_i < l_p1 and l_i < l_p2:
                    recent_lows.append(l_i)
...
def is_micro_bos_down(candles_5m: List[dict]) -> bool:
    ...
    try:
        ...
        for i in range(max(2, len(candles_5m)-6), len(candles_5m)-1):
            try:
                h_i  = float(candles_5m[i]['h'])
                h_p1 = float(candles_5m[i-1]['h'])
                h_p2 = float(candles_5m[i-2]['h'])
                if h_i > h_p1 and h_i > h_p2:
                    recent_highs.append(h_i)
            except (KeyError, ValueError, IndexError):
                continue
        if not recent_highs:
            return False
        ...
        try:
            ...
            for setup_id, setup in setups_snapshot:
                ...
                try:
                    ...
                    logger.debug(f"Trigger check error {setup_id}: {_te}")
                    triggered, reason = False, "trigger_check_error"
            ...
            logger.error(f"monitor_pending_setups error: {e}", exc_info=True)
            time.sleep(5)
