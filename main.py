...
def init_db():
    ...
    # ===== MIGRASI: jaga-jaga db lama (V7/V8) belum punya kolom V9/V10 =====
    # CREATE TABLE IF NOT EXISTS gak ngubah tabel yang udah ada, jadi kolom baru
    # di schema atas cuma kepake buat db yang baru dibuat. ALTER ini nutup gap-nya.
    _migrations = [
...
def compute_market_breath_v10() -> Dict[str, float]:
    """V10: Advanced market breath dengan Participation, Leadership, Dispersion, Rotation"""
    global _breath_cache
    with _breath_lock:
        now = time.time()
        if _breath_cache and now - _breath_cache.get("ts", 0) < 60:
...
def get_context_snapshot(coin: str = "BTC") -> ContextSnapshot:
    """V10: Get context snapshot dengan cache (TTL)"""
...
# ============================================================
# PART 10 – EVENT RISK + REACTION ENGINE (V10 CORE)
# ============================================================

def get_event_risk_adjustment() -> Dict[str, float]:
    """Return adjustment factors based on event risk (V10: structured)"""
...
def set_event_risk_v10(importance: int, expected_vol: int, scope: str, bias: str, label: str, ts: float = None):
    """Set event risk dengan struktur lengkap (V10)"""
    if ts is None:
        ts = time.time()
    with _event_risk_lock:
...
def compute_reaction(event_risk: EventRisk, btc_move: float, vol_spike: float) -> MarketReaction:
    """Compute market reaction to event"""
...
def get_reaction_adjustment() -> Dict[str, float]:
    """Return adjustment based on latest reaction"""
    with _reaction_lock:
...
def update_reaction_history(reaction: MarketReaction):
    """Update reaction history dan log ke DB"""
    with _reaction_lock:
...
def get_intent_success_rate(coin: str, intent: str) -> float:
    """Hitung success rate untuk intent tertentu di coin ini"""
    with _intent_memory_lock:
        if coin not in _intent_memory:
            return 0.5
...
def get_intent_success_rate_all(intent: str) -> float:
    """Hitung success rate untuk intent tertentu across semua coins"""
    with _intent_memory_lock:
        cutoff = time.time() - TUNABLE["INTENT_MEMORY_HOURS"] * 3600
        all_entries = []
...
# ============================================================
# PART 12 – EXECUTION MODE V10 (5 Mode)
# ============================================================
...
def get_execution_mode_v10(context: ContextSnapshot, reaction: Optional[MarketReaction],
    ...
    """
    Tentukan execution mode berdasarkan context, reaction, intent memory, dan event risk
    Return: (mode, adjustments)
    """
    # DEFENSIVE: event risk tinggi atau market chaos
    ...
    # AGGRESSIVE: shock tinggi + tension tinggi + reaction kuat
...
def get_mode_adjustment(mode: ExecutionMode) -> Dict[str, float]:
    """Return adjustment factors for each mode"""
    adjustments = {
...
# ============================================================
# PART 13 – REGIME INERTIA + CONTEXT WITH CONFIDENCE
# ============================================================

def get_regime_with_inertia(coin: str) -> Tuple[str, float]:
    """Return regime dengan penalty kalau terlalu sering ganti"""
...
def get_context_with_confidence(coin: str, confidence: float) -> ContextSnapshot:
    """Context cache dengan TTL berdasarkan confidence DAN entropy"""
    with _CONTEXT_CACHE_LOCK:
        now = time.time()
        bucket = int(confidence / 10) * 10
        cache_key = f"{coin}_{bucket}"
        if cache_key in _CONTEXT_CACHE:
            cached, ts, cached_entropy = _CONTEXT_CACHE[cache_key]
            current_entropy = compute_market_entropy_v7(coin, None)
            if confidence > 70:
                base_ttl = 10
            elif confidence > 40:
                base_ttl = 5
                ...
                base_ttl = 3
            entropy_factor = 1.0 + (current_entropy / 100) * 0.5
            ttl = base_ttl / entropy_factor
            if now - ts < ttl:
                return cached
    ctx = get_context_snapshot(coin)
    current_entropy = compute_market_entropy_v7(coin, None)
    with _CONTEXT_CACHE_LOCK:
        _CONTEXT_CACHE[cache_key] = (ctx, now, current_entropy)
    return ctx
    

# ============================================================
# PART 14 – SNAPSHOT + DATA FUNCTIONS
# ============================================================
...
def refresh_snapshot():
    ...
    with _snapshot_lock:
        if _last_snapshot and now - _last_snapshot.timestamp < ttl:
            return
        try:
            meta = info.meta_and_asset_ctxs()
            mids = {}
            ...
            _last_snapshot = MarketSnapshot(timestamp=now, mids=mids, oi=oi, funding=funding)
...
def get_bias_4h_advanced(coin: str) -> Tuple[str, float, float]:
    """
    V10: Return (bias, strength, stability)
    - bias: BULLISH / BEARISH / NEUTRAL
    - strength: 0-100 (seberapa kuat bias)
    - stability: 0-1 (seberapa stabil bias dalam 5 candle terakhir)
    """
...
# ============================================================
# PART 16 – ZONE MEMORY DECAY + FVG FILL SPEED + OI PERSISTENCE
# ============================================================
...
# ============================================================
# PART 17 – BELIEF STATE + PREDICTION QUALITY
# ============================================================
...
# ============================================================
# PART 18 – ENTROPY + DECISION ENERGY
# ============================================================
...
# ============================================================
# PART 19 – EXECUTION MODE BLEND + FATIGUE + FILTER GRADIENT
# ============================================================
...
# ============================================================
# PART 20 – INTENT ENGINE + WHY NOT + POSITION SIZE + THESIS GENERATOR
# ============================================================
...
# ============================================================
# PART 21 – EVENT DETECTION (LIQUIDITY, OB, FVG)
# ============================================================
...
# ============================================================
# PART 22 – EVENT DETECTION (OB_FLOW, FVG_FLOW, VACUUM, CLUSTER)
# ============================================================
...
# ============================================================
# PART 23 – SCORING + MOMENTUM + SL/TP
# ============================================================
...
# ============================================================
# PART 24 – LAYER 1: OBSERVE MARKET
# ============================================================
...
# ============================================================
# PART 25 – LAYER 2: BUILD THESIS (dengan Macro Hierarchy V10)
# ============================================================
...
def build_thesis(obs: Dict) -> Optional[Dict]:
    ...
    # V10: Macro bias override dengan strength + stability
    if bias_4h == "BEARISH" and event.direction == "LONG":
        # Downgrade berdasarkan strength
        downgrade = 20 if bias_strength > 70 else 10
        if bias_stability < 0.4:
            # Kalau bias tidak stabil, downgrade lebih kecil
            downgrade = int(downgrade * 0.6)
        logger.debug(f"{coin}: 4H BEARISH (str:{bias_strength:.0f}, stab:{bias_stability:.2f}), LONG downgraded")
        event.strength = max(30, event.strength - downgrade)
        event.confidence = max(30, event.confidence - downgrade * 0.75)
    ...
    # BELIEF (dengan macro bias adjustment)
    ...
    # V10: Macro bias turunkan belief score (dengan strength)
    if bias_4h == "BEARISH" and event.direction == "LONG":
...
# ============================================================
# PART 26 – LAYER 3: COMPUTE CONFIDENCE + HELPER FUNCTIONS
# ============================================================
...
# ============================================================
# PART 27 – LAYER 4: EXECUTE DECISION (V10)
# ============================================================
...
def execute_decision(coin: str, thesis_data: Dict, confidence_data: Dict, 
    ...
    # ===== V10: REACTION ENGINE =====
    # Ambil BTC move terbaru dari snapshot
    snapshot = get_snapshot()
    btc_move = 0.0
    ...
    if breath_v10.get("participation", 0.5) < 0.4 and coin != "BTC":
        # Low participation = market narrow → alt lebih riskan
        position_size_mult *= 0.7
    if breath_v10.get("leadership", 0) > 2 and coin != "BTC" and event.direction == "LONG":
        # Leadership strong = top coins outperform → alt long bisa ikut
        position_size_mult *= 1.1
    if breath_v10.get("rotation", 0) > 1 and coin not in ["BTC", "ETH"]:
        # Rotation ke small cap → alt bisa lebih kuat
        position_size_mult *= 1.15
    ...
    # V10: Tambahkan intent success ke why_not
...
# ============================================================
# PART 28 – CHECK ENTRY V10 + GLOBAL MARKET INTENT + EVALUATE SIGNAL
# ============================================================

def classify_global_market_intent_v10(context: ContextSnapshot, breath: Dict[str, float]) -> Tuple[str, float]:
    """
    V10: Global market intent dengan advanced breath metrics
    Return: (intent, score)
    """
    score = 0.0
    intent = "NEUTRAL"
    ...
    # Trending detection (regime + breath)
    if context.regime in ("TRENDING_UP", "TRENDING_DOWN"):
        intent = "TRENDING"
        score += 30
    
    # Participation: market luas atau sempit?
    if breath.get("participation", 0.5) > 0.7:
        score += 15  # Market luas = trending lebih valid
    elif breath.get("participation", 0.5) < 0.3:
        score -= 10  # Market sempit = hati-hati
    
    # Leadership: top coins vs bottom
    if breath.get("leadership", 0) > 2:
        score += 15  # Top coins kuat = bullish
    elif breath.get("leadership", 0) < -2:
        score -= 15  # Top coins lemah = bearish
    
    # Rotation: small cap vs large cap
...
def validate_local_vs_global_v10(obs: Dict, global_intent: str, global_score: float, breath: Dict[str, float]) -> bool:
    """V10: Validate local setup vs global intent dengan advanced breath"""
    event = obs["best_event"]
    ...
    # ROTATION: prefer small cap setups
    if global_intent == "ROTATION":
        if obs["coin"] not in ["BTC", "ETH"]:
            return True  # Rotation ke alt
        return False
    
    # TRANSITION: hati-hati, prefer flow-based
    ...
    # EVENT_RISK: prefer defensive setups (OB)
...
# ============================================================
# PART 29 – ENGINE LOOPS + DB QUEUE WRITER + GRACEFUL SHUTDOWN
# ============================================================
...
def state_engine_update_v10():
    ...
    with _event_risk_lock:
        if _EVENT_RISK_DATA:
            latest_event = max(_EVENT_RISK_DATA, key=lambda e: e.ts)
            # Check if event is still relevant (< 3 hours ago)
...
# ============================================================
# PART 30 – TELEGRAM BOT V10 (UI Upgrade)
# ============================================================
...
def send_alert_v10(alert: dict):
    ...
    def get_progressive_cooldown(c: str) -> int:
        ...
        # V10: cooldown dipengaruhi execution mode
...
# ============================================================
# PART 31 – BOT COMMANDS V10 (UI Upgrade)
# ============================================================
...
# ============================================================
# PART 32 – BOT COMMANDS (Journal, Belief, Fatigue, Prediction, Traces)
# ============================================================
...
# ============================================================
# PART 33 – BOT COMMANDS (Status, Analytics, Entry, Warroom, Stopalert, Health, Intel)
# ============================================================
...
# ============================================================
# PART 34 – MAIN + SIGNAL HANDLER + GRACEFUL SHUTDOWN
# ============================================================
