...
def log_velocity_trace(
    ...
    """Log per-decision decision-level metrics. score/threshold may be None
    (e.g. for skip/reject events that occur before scoring) — these are
    rendered as the literal string 'None' rather than coerced to 0.0,
    so downstream parsers (cmd_velocity) can tell "no score" apart from
    "score of zero" instead of having every skip silently pollute the
    median with zeros."""
    try:
        pipe = get_pipeline_metrics()
...
def emit_velocity_skip(
    ...
    """
    P4.0 — Emit a VELOCITY_TRACE for skip/reject gates WITH whatever
    ...
    zeros. Use None for score/threshold when not yet computed at this
