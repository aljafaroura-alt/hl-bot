...
# _hypothesis_store: disimpan langsung ke database, tidak perlu in-memory store
...
def add_shadow_decision(signal_id, coin, direction, entry, sl, tp, eval_delay: int = BASE_EVALUATION_DELAY):
    with _shadow_lock:
        _shadow_decisions[signal_id] = {
            ...
            "mfe": 0.0, "mae": 0.0, "eval_delay": eval_delay
        }
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def update_data_integrity_history(coin: str, oi_usd: float, funding_pct: float, price: float):
    with _data_integrity_lock:
        ...
        # Hanya append jika nilai bukan 0 (partial update tidak menimpa data sebelumnya)
        if oi_usd != 0:
            _oi_values[coin].append((now, oi_usd))
        if funding_pct != 0:
            _funding_values[coin].append((now, funding_pct))
        if price != 0:
            _price_values[coin].append((now, price))
...
# (implementasi ada di bawah, dekat fungsi-fungsi evaluasi)
...
def get_zone_penalty(coin: str, zone_type: str, low: float, high: float) -> int:
    ...
    with _zone_memory_lock:
        ...
        if density > 5:
            ...
            penalty = 0
        # Absolute touch_count penalty: zona yg terlalu sering disentuh = exhausted
        if touch_count > 20:
            penalty += 20
        elif touch_count > 10:
            penalty += 10
...
def check_entry_alert(coin: str, mark: float, master_candles: Dict) -> Optional[dict]:
    ...
    try:
        ...
        _shadow_eval_delay = get_evaluation_delay(atr_pct, rr, market_regime)
        add_shadow_decision(shadow_id, coin, best_event.direction, mark, sl, tp, eval_delay=_shadow_eval_delay)
    ...
    while not _shutdown_event.is_set():
        with _shadow_lock:
            ...
            for sid, shadow in list(_shadow_decisions.items()):
                eval_delay = shadow.get("eval_delay", BASE_EVALUATION_DELAY)
                if not shadow["evaluated"] and now - shadow["timestamp"] > eval_delay:
                    try:
                        coin = shadow["coin"]
                        entry = shadow["entry"]
                        sl = shadow["sl"]
