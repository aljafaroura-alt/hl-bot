...
def get_adaptive_threshold(market_regime: str, entropy_market: int, recent_win_rate: float, execution_count: int = 0) -> int:
    ...
    if execution_count > 3:
        base += min(10, execution_count * 2)  # Fatigue
    
    final = max(50, min(95, base))
    
    logger.debug(f"📊 P2: THRESHOLD: base={base} (regime={market_regime}, entropy={entropy_market}, wr={recent_win_rate:.0%}, exec={execution_count}) → final={final}")
...
def get_price_1h_ago(coin: str) -> Optional[float]:
    ...
    """
    with _price_history_lock:
        if coin not in _price_history_1h or len(_price_history_1h[coin]) < 5:
            return None
        ...
        # Cari data paling dekat dengan 1 jam lalu
        best_ts = None
        best_price = None
        for ts, price in history:
            if ts <= cutoff:
                # Ambil yang paling baru sebelum cutoff
...
def get_oi_pattern_v11(coin: str) -> Tuple[str, float, float]:
    """
    OI Pattern detector multi-timeframe: 30m, 1h, 4h, 24h
    ...
    NEUTRAL   : campuran/flat = skip
    UNKNOWN   : data kurang = skip
    
    Return: (pattern, oi_4h_growth_pct, coverage_ratio)
    """
    try:
        # Import dari main.py context (global variable)
        import sys
        main = sys.modules.get('__main__')
        _oi_history = getattr(main, '_oi_history', {})
        _oi_lock = getattr(main, '_oi_lock', threading.RLock())
    except Exception:
        return "UNKNOWN", 0.0, 0.0
    
    with _oi_lock:
        if coin not in _oi_history or len(_oi_history[coin]) < 3:
            return "UNKNOWN", 0.0, 0.0
    ...
    # Ambil OI di 4 timeframe
    oi_30m, _, cov_30m = get_oi_at_timeframe(history, 1800)
    oi_1h, _, cov_1h = get_oi_at_timeframe(history, 3600)
    oi_4h, _, cov_4h = get_oi_at_timeframe(history, 14400)
    oi_24h, _, cov_24h = get_oi_at_timeframe(history, 86400)
    
    # Coverage ratio: rata-rata coverage dari semua timeframe yang ada
    coverages = [c for c in [cov_30m, cov_1h, cov_4h, cov_24h] if c > 0]
    total_coverage = sum(coverages) / len(coverages) if coverages else 0.0
    
    # Minimum coverage: 0.4 (biar ga junk)
    # Tapi kalau data baru mulai warmup, kasih chance lebih rendah
    if len(history) < 10:
        min_coverage = 0.2  # warmup: lebih toleran tapi confidence rendah
    else:
        min_coverage = 0.4
    
    if total_coverage < min_coverage:
        return "UNKNOWN", 0.0, total_coverage
    
    # Threshold adaptif: 2% default, tapi kalau OI kecil bisa lebih ketat
    TH = 2.0
    
    # Hitung growth (kalau data tersedia)
    g_30m = (oi_now - oi_30m) / max(oi_30m, 0.001) * 100 if oi_30m > 0 and cov_30m > 0 else 0
    g_1h = (oi_now - oi_1h) / max(oi_1h, 0.001) * 100 if oi_1h > 0 and cov_1h > 0 else 0
    g_4h = (oi_now - oi_4h) / max(oi_4h, 0.001) * 100 if oi_4h > 0 and cov_4h > 0 else 0
    g_24h = (oi_now - oi_24h) / max(oi_24h, 0.001) * 100 if oi_24h > 0 and cov_24h > 0 else 0
    ...
    if is_up_30m and is_up_1h and is_up_4h and not is_up_24h:
        ...
        return "SPIKE", g_30m, total_coverage  # short-term blip, waspada
...
# ============================================================
# GATE 3: OI THRESHOLD PERCENTILE (Bukan Hardcode)
# ============================================================

def get_oi_percentile_threshold(snapshot, min_percentile: float = 35) -> float:
    """
    Threshold OI berdasarkan percentile dari distribusi pasar saat ini.
    
    Kenapa percentile bukan absolut:
    - Bear market: semua OI turun, $5M bisa jadi top tier
    - Bull market: altcoin kecil pun OI bisa >$10M
    - Percentile selalu adaptive
    
    min_percentile=35 artinya: ambil coin yang OI-nya di atas 35% terendah
    (buang 35% paling kecil, ambil 65% teratas)
    """
    if not snapshot or not snapshot.oi:
        return 500_000  # fallback
    
    oi_values = sorted([v for v in snapshot.oi.values() if v > 0])
    if not oi_values:
        return 500_000
    
    idx = int(len(oi_values) * min_percentile / 100)
    idx = max(0, min(idx, len(oi_values) - 1))
    threshold = oi_values[idx]
    
    # Hard floor & ceiling untuk sanity
    threshold = max(200_000, min(50_000_000, threshold))
    return threshold


# ============================================================
# GATE 4: NARRATIVE BOOST (Dari Preliminary Pool, Bukan Seluruh Market)
# ============================================================

# Sector map: update kalau ada listing baru di Hyperliquid
_SECTOR_MAP: Dict[str, str] = {
    # L1/L2
    "BTC": "BTC_ECO", "ORDI": "BTC_ECO",
    "ETH": "ETH_ECO", "STETH": "ETH_ECO",
    "SOL": "SOL_ECO", "JUP": "SOL_ECO", "PYTH": "SOL_ECO", "RAY": "SOL_ECO",
    "SUI": "L1_ALT", "APT": "L1_ALT", "SEI": "L1_ALT", "AVAX": "L1_ALT",
    "ARB": "L2", "OP": "L2", "STRK": "L2", "MANTA": "L2",
    # AI
    "TAO": "AI", "FET": "AI", "RNDR": "AI", "AKT": "AI", "WLD": "AI",
    "AGIX": "AI", "OCEAN": "AI", "GRT": "AI",
    # DeFi
    "UNI": "DEFI", "AAVE": "DEFI", "LINK": "DEFI", "MKR": "DEFI",
    "CRV": "DEFI", "SNX": "DEFI", "DYDX": "DEFI",
    # Meme
    ...
    # Infra
    "DOT": "INFRA", "ATOM": "INFRA", "NEAR": "INFRA", "FIL": "INFRA",
    "AR": "INFRA", "LPT": "INFRA",
    # Perp/Exchange
    "HYPE": "PERP_DEX", "GMX": "PERP_DEX",
}


def get_coin_sector(coin: str) -> Optional[str]:
    return _SECTOR_MAP.get(coin.upper())


def get_narrative_boost_v11(coin: str, preliminary_pool: List[str]) -> float:
    """
    Narrative boost HANYA dari preliminary pool.
    
    Kenapa bukan dari seluruh market:
    - Kalau dari seluruh market: ETH, BTC, SOL selalu punya sektor yang rame
    - Preliminary pool = kandidat yang udah lulus gate OI
    - Kalau 3+ coin dari sektor yang sama lulus gate OI = sektornya lagi hot
    ...
    if same_sector_count >= 3:
        return 0.20  # sektor lagi rame banget
...
_SECTOR_MAP = {
    # DeFi Layer1
    "SOL": "LAYER1", "ETH": "LAYER1", "ARB": "LAYER1", "OP": "LAYER1", "AVAX": "LAYER1",
    "FTM": "LAYER1", "MATIC": "LAYER1", "BLAST": "LAYER1",
    # DeFi
    "UNI": "DEFI", "AAVE": "DEFI", "CRV": "DEFI", "LDO": "DEFI", "GMX": "DEFI",
    "MKR": "DEFI", "LIDO": "DEFI", "DYDX": "DEFI",
    # AI/ML
    "RENDER": "AI", "AGIX": "AI", "ARKM": "AI", "OCEAN": "AI", "FET": "AI",
    # Memecoin
...
def get_coin_sector(coin: str) -> Optional[str]:
    """Lookup coin's sector from mapping."""
    return _SECTOR_MAP.get(coin.upper())
...
def execute_decision(coin: str, thesis_data: Dict, confidence_data: Dict,
    ...
    if not conviction_data["is_qualified"]:
        reason = f"conviction {conviction_data['conviction']:.0f} < 45"
    ...
    if not micro_confirmed:
        ...
        record_opportunity_rejected(coin, f"micro_structure_{micro_score}")
        update_fatigue_memory(event.type)
        return None
...
def check_entry_alert_v10_phase1(coin: str, mark: float, master_candles: Dict) -> Optional[dict]:
    ...
    try:
        ...
        if not thesis_data or thesis_data.get("status") == "REJECT":
            ...
            logger.debug(f"❌ THESIS REJECT {coin}: {reason}")
            record_opportunity_rejected(coin, reason)
...
def build_scan_universe_v2(min_vol: int = 5_000_000, base_limit: int = 20) -> List[str]:
    ...
    oi_changes = {coin: get_oi_roc(coin) for coin in candidates}
...
_NARRATIVE_MAP: Dict[str, List[str]] = {
    "MEME": ["DOGE", "WIF", "BONK", "FARTCOIN", "PUMP"],
    "AI": ["TAO", "FET", "RNDR", "AKT", "WLD"],
    "L2": ["ARB", "OP", "STRK"],
    "DEFI": ["UNI", "AAVE", "LINK", "MKR", "CRV"],
    "SOL_ECO": ["SOL", "JUP", "PYTH", "RAY"],
    "BTC_ECO": ["BTC", "ORDI"],
    "L1_ALT": ["ETH", "AVAX", "SUI", "XRP", "HYPE"],
}
...
def get_top_narrative() -> Tuple[str, float]:
    """Cari sektor dengan rata-rata OI change tertinggi (dinamis, bukan hardcode pemenang)."""
    snapshot = get_snapshot()
    if not snapshot:
        return "UNKNOWN", 0.0

    sector_scores = {}
    for sector, coins in _NARRATIVE_MAP.items():
        valid_coins = [c for c in coins if c in snapshot.mids]
        if not valid_coins:
            continue
        oi_changes = [get_oi_roc(c) for c in valid_coins]
...
def get_top_narrative_with_exploration() -> Tuple[str, float]:
    """25% chance explore sektor random (anti feedback-loop: MEME menang -> scan MEME ->
    OI MEME naik -> MEME menang lagi -> stuck)."""
    if random.random() < 0.25:
        sector = random.choice(list(_NARRATIVE_MAP.keys()))
        with _sector_history_lock:
...
def get_oi_flow_coins(snapshot: MarketSnapshot, limit: int = 6) -> List[str]:
    ...
    candidates = [c for c, oi_usd in snapshot.oi.items() if oi_usd >= 1_000_000]
    if not candidates:
        return []

    oi_changes = {c: get_oi_roc(c) for c in candidates}
    oi_rank_map = percentile_rank(list(oi_changes.values()))
...
def get_narrative_coins(snapshot: MarketSnapshot, limit: int = 3) -> List[str]:
    """Narrative bucket: coin dari sektor terpanas (dengan exploration anti-lock)."""
    sector, score = get_top_narrative_with_exploration()
    if sector == "UNKNOWN" or score < 5:
        return []
    coins = [c for c in _NARRATIVE_MAP.get(sector, []) if c in snapshot.mids]
    if not coins:
        return []
    scored = [(c, get_oi_roc(c)) for c in coins]
    scored.sort(key=lambda x: x[1], reverse=True)
    return [c[0] for c in scored[:limit]]
...
def build_candidate_pool_v11_final(max_candidates: int = 12) -> List[str]:
    ...
    """
    Discovery V11 Final: Capital Rotation Detector
    
    Flow:
    1. Snapshot (1 API call, udah ada)
    2. Filter OI threshold percentile
    3. OI Pattern classification (dari _oi_history, no API)
    4. Dislocation score (dari _price_history_1h, no API)
    5. Narrative boost (dari preliminary pool)
    6. Memory decay (anti-repeat)
    7. Alpha sentinel (BTC always in)
    
    ZERO extra API calls vs V10.
    """
    try:
        # Ambil snapshot (udah di-cache, no extra API call)
        import sys
        main_module = sys.modules.get('__main__')
        get_snapshot_fn = getattr(main_module, 'get_snapshot', None)
        get_oi_roc_fn = getattr(main_module, 'get_oi_roc', None)
        get_coin_selection_count_fn = getattr(main_module, 'get_coin_selection_count', None)
        _candidate_history_ref = getattr(main_module, '_candidate_history', {})
        _candidate_history_lock_ref = getattr(main_module, '_candidate_history_lock', threading.RLock())
        logger_ref = getattr(main_module, 'logger', None)
        
        snapshot = get_snapshot_fn() if get_snapshot_fn else None
        
        if not snapshot or not snapshot.mids:
            return ["BTC", "ETH", "SOL", "ARB", "OP"]
        
        # --- Gate: OI threshold percentile ---
        oi_threshold = get_oi_percentile_threshold(snapshot, min_percentile=35)
        if logger_ref:
            logger_ref.debug(f"🔍 Discovery V11: OI threshold={oi_threshold/1e6:.2f}M (35th pct)")
        
        # --- Score tiap coin ---
        scores: Dict[str, float] = {}
        pattern_log: Dict[str, Tuple[str, float, float, float]] = {}  # untuk debug
        ...
        for coin in list(snapshot.mids.keys()):
            ...
            # Gate 2: OI Pattern (coverage check built-in)
            pattern, oi_growth, coverage = get_oi_pattern_v11(coin)
            
            if pattern in ("UNKNOWN", "NEUTRAL", "LATE"):
                continue  # skip yang ga menarik
            
            # Gate 1 (sudah digabung): Dislocation
            dislocation, dis_confidence = get_dislocation_score_v11(coin, snapshot)
            
            # --- Base score ---
            if pattern == "EARLY":
                # Paling menarik: OI baru mulai naik, harga belum
                base_score = 70 + min(30, max(0, dislocation / 2))
            elif pattern == "MOMENTUM":
                # Udah jalan tapi masih worth: OI naik konsisten
                base_score = 50 + min(25, max(0, dislocation / 2))
            elif pattern == "SPIKE":
                # Berisiko: bisa short squeeze atau rumor
                base_score = 35 + min(15, max(0, dislocation / 2))
            else:
                base_score = 10
            
            # Coverage adjustment: data kurang = confidence kurang
            base_score *= coverage
            
            # Dislocation confidence adjustment
            base_score *= (0.7 + 0.3 * dis_confidence)
            
            # Memory decay: coin yang sering kepilih dapat penalty
            if get_coin_selection_count_fn:
                n = get_coin_selection_count_fn(coin)
                base_score *= math.exp(-0.15 * n)
            
            scores[coin] = base_score
            pattern_log[coin] = (pattern, oi_growth, coverage, dislocation)
        
        if not scores:
            # Fallback: minimal ada BTC
            if logger_ref:
                logger_ref.warning("Discovery V11: no coins passed OI gates, using fallback")
            return ["BTC", "ETH", "SOL"]
        
        # --- Sort untuk preliminary pool ---
        sorted_coins = sorted(scores.items(), key=lambda x: x[1], reverse=True)
        preliminary_pool = [c for c, _ in sorted_coins[:max_candidates + 5]]
        
        # --- Gate 3: Narrative boost dari preliminary pool ---
        final_scores: Dict[str, float] = {}
        for coin in preliminary_pool:
            if coin not in scores:
                continue
            boost = get_narrative_boost_v11(coin, preliminary_pool)
            final_scores[coin] = scores[coin] * (1 + boost)
        ...
        with _candidate_history_lock_ref:
            for coin in candidates:
                _candidate_history_ref[coin] = _candidate_history_ref.get(coin, 0) + 1
        
        # --- Logging ---
        if logger_ref:
            logger_ref.info(
                f"🔍 DISCOVERY V11 FINAL: {len(candidates)} candidates "
                f"(OI_threshold={oi_threshold/1e6:.1f}M)"
            )
            for coin, score in final_sorted[:8]:
                pattern, oi_growth, coverage, dis = pattern_log.get(coin, ("?", 0, 0, 0))
                boost = get_narrative_boost_v11(coin, preliminary_pool)
                logger_ref.debug(
                    f"  {coin}: {score:.0f} | {pattern} "
                    f"oi_4h={oi_growth:+.1f}% dis={dis:+.1f} "
                    f"cov={coverage:.2f} boost={boost:.0%}"
                )
        ...
        import traceback
        if 'logger_ref' in dir() and logger_ref:
            logger_ref.error(f"build_candidate_pool_v11_final error: {e}\n{traceback.format_exc()}")
        return ["BTC", "ETH", "SOL", "ARB", "OP"]
...
def process_candidates_deep(candidates: List[str], snapshot: MarketSnapshot) -> Tuple[List[dict], int]:
    ...
    Snapshot di-share (1x), bukan dipanggil ulang per coin."""
    results = []
    scan_count = 0

    for coin in candidates:
        if get_api_used() >= API_BUDGET_PER_CYCLE:
            logger.debug("⏸️ V11: API budget exhausted, stopping deep analysis")
            break
        ...
        if not can_call_api():  # global 429-brake tetap dihormati
            logger.debug(f"⏳ Global API cooldown active, skip {coin}")
            continue

        if can_call_api_endpoint("candles"):
    ...
    logger.info(f"📊 V11 Deep analysis: {len(results)} alerts from {scan_count} coins scanned, {get_api_used()} API calls used")
        ...
        try:
            if alert.get("tp_scaled"):
                TRADE_MANAGER.add_position(
                    signal_id=alert.get("signal_id", f"{alert['coin']}_{int(time.time())}"),
                    coin=alert["coin"],
...
def _build_compact_alert(alert: dict) -> str:
    ...
    compact = (
        f"{direction_emoji} *{alert['coin']} {alert['direction']}*\n"
