...
def log_velocity_observer(coin: str, event, candles_5m: List[dict]) -> None:
    ...
    vol_score = min(100, (vol_spike - 0.8) * 50)
    ...
    logger.debug(
        f"⚡ VELOCITY OBSERVER {coin}: comp={composite:.0f} ({status}) | "
        f"F:{delta_score:.0f} V:{vol_score:.0f} OI:{oi_score:.0f} A:{accept_score:.0f}"
    )
...
def get_rejection_reason_counts(window_minutes: int = 60) -> Dict[str, int]:
    """Count rejections by reason from decision journal (last N minutes)."""
    ...
    with _journal_lock:
        for entry in _decision_journal:
            ...
            if entry.decision == "REJECT":
                reason = entry.extra.get("reason", "unknown") if entry.extra else "unknown"
...
def compute_confidence(thesis_data: Dict) -> Optional[Dict]:
    ...
    logger.debug(
        f"[RR {coin}] rr={rr:.2f} base_rr={base_rr:.2f} "
        f"entropy={entropy_market} entropy_mult={entropy_mult:.2f} "
        f"final_min_rr={min_rr:.2f} regime={thesis_data['market_regime']}"
    )
    if rr < min_rr:
        logger.debug(
    ...
    breath_snapshot = get_market_breath()
...
def get_oi_zscore(coin: str) -> float:
    ...
    """
    oi_history = _oi_history.get(coin, deque(maxlen=60))  # Last 60 snapshots (~5min)
...
def get_velocity_score(coin: str, direction: str) -> Tuple[int, List[str]]:
    ...
    logger.debug(
        f"⚡ VELOCITY {coin} {direction}: score={velocity_score} | {', '.join(reasons)}"
    )
...
def execute_decision(coin: str, thesis_data: Dict, confidence_data: Dict,
    ...
    if clarity["decision_quality"] < 0.55:
    ...
    calibration_penalty = (raw_conf - calibrated_conf) * 0.3 if raw_conf > calibrated_conf else 0
...
📚 <b>Memory</b>: {journal_size} decisions
...
def cmd_health(m):
    ...
    # === TOP REJECTION REASONS ===
    with _opportunity_lock:
        rejections = _opportunity_stats.get("rejection_reasons", {})
        top_reject = sorted(rejections.items(), key=lambda x: x[1], reverse=True)[:3]
...
def get_universe_v10(limit: int = 20) -> List[str]:
