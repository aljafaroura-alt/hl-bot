...
class TradeManager:
    ...
    def _update_trailing_stop(self, pos: OpenPosition, current_price: float):
        ...
        if pos.trailing_activated:
            # P4.56: trail distance dinamis dari volatilitas, bukan angka tetap.
            # Priority: (1) explicit override kalau ada caller yang set manual,
            # (2) ATR snapshot di entry time (paling representatif — reflect
            #     volatilitas struktural coin ini saat posisi dibuka),
            # (3) fallback ke risk distance posisi itu sendiri (entry vs sl),
            #     karena itu sudah structure-based juga (lihat calculate_sltp_advanced),
            #     bukan konstanta independen dari posisi.
            if pos.trail_distance_pct > 0:
                trail_pct = pos.trail_distance_pct
            elif pos.entry_atr_pct > 0:
                trail_pct = pos.entry_atr_pct * 0.8  # rasio terhadap ATR, bukan ATR mentah
            else:
                risk_pct = abs(pos.entry - pos.sl) / max(pos.entry, 0.01) * 100
                trail_pct = max(0.3, risk_pct * 0.5)  # setengah dari risk awal, floor kecil cuma anti-zero
    ...
    def _update_profit_lock(self, pos: OpenPosition, current_price: float):
        ...
        PAKAI: lock_ratio = min(0.3, MFE/10)
        ...
        lock_ratio = min(0.3, mfe_pct / 10)  # MFE 3% → 0.3, MFE 0.5% → 0.05
        lock_distance = pos.entry * (lock_ratio / 100)

        if pos.direction == "LONG":
            new_sl = max(pos.sl, pos.entry + lock_distance)
            if new_sl > pos.sl:
                pos.sl = new_sl
                logger.info(f"🔒 PROFIT LOCK {pos.coin}: SL → {new_sl:.4f} (lock_ratio={lock_ratio:.2f}, mfe={mfe_pct:.2f}%)")
        else:
    ...
    try:
        ...
        if hypothesis_validated is not None:
            c.execute('''UPDATE signals SET evaluated=1, outcome=?, pnl=?, exit_price=?, exit_time=?, 
                         ...
                         feedback_early=?, feedback_shape=?, feedback_mature=? WHERE signal_id=?''',
                      (outcome, pnl, exit_price, int(time.time()), mfe, mae,
                       1 if hypothesis_validated else 0,
                       exit_eff, data_source, regime, cache_age,
                       feedback_early, feedback_shape, feedback_mature, signal_id))
        else:
            c.execute('''UPDATE signals SET evaluated=1, outcome=?, pnl=?, exit_price=?, exit_time=?, 
                         mfe=?, mae=?,
                         exit_eff=?, source=?, regime=?, cache_age=?,
...
def compute_suggested_leverage(coin: str, entry: float, sl: float, position_size_mult: float) -> Dict[str, Any]:
    """P4.56: leverage yang DISARANKAN buat eksekusi manual di exchange —
    bot ini read-only (cuma Info, gak Exchange), jadi ini bukan kontrol
    order, murni informasi buat keputusan manual lo.

    Tidak ada angka leverage hardcode di sini sama sekali:
    - native_cap: dari Hyperliquid sendiri (maxLeverage di meta(), BTC/ETH
      biasanya jauh lebih tinggi dari coin kecil — itu KEPUTUSAN EXCHANGE,
      bukan list yang gw tulis manual).
    - suggested: rasio dari risk_pct posisi ini (SL distance, makin rapat
      makin mendekati native_cap karena struktur lebih confident) DIKALI
      position_size_mult yang SUDAH dihitung sistem dari conviction/regime/
      cooldown (jadi conviction tinggi → leverage disarankan ikut naik,
      bukan variable independen baru).
    Hasil akhir tetap dijepit ke native_cap exchange — gak pernah nyaranin
    leverage di atas yang exchange izinkan.
    """
    ...
    # Rasio dasar: berapa kali risk_pct dibutuhkan buat "mengisi" headroom
    # sampai native_cap. SL rapat (risk_pct kecil) → rasio besar → mendekati
    # cap. SL lebar → rasio kecil → leverage disarankan ikut kecil.
    # 100 di sini bukan konstanta ajaib — itu konversi unit (risk_pct dalam
    # persen, leverage dalam x), supaya risk_pct=1% pas SL rapat ~ pakai
    # porsi besar dari cap yang tersedia.
    headroom_ratio = min(1.0, (100 / max(risk_pct, 0.01)) / native_cap)
    raw_suggested = native_cap * headroom_ratio * max(0.1, min(2.0, position_size_mult))
...
def calculate_sltp_advanced(coin: str, mark: float, direction: str, event: TradeEvent,
    ...
    """P4.56: TP target structure-first. Urutan fallback (tiap layer cuma
    dipakai kalau layer sebelumnya gak punya data — bukan dipilih manual):
