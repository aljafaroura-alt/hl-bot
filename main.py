...
def get_adaptive_threshold(market_regime: str, entropy_market: int, recent_win_rate: float, execution_count: int = 0) -> int:
    ...
    # === P5: HYBRID INVENTORY — managed positions + DB sebagai safety bound ===
    ...
    try:
        ...
        db_open = managed_open

    if managed_open < 10 and db_open > 50:
        # Restore/recovery kemungkinan belum lengkap → pakai DB count (lebih aman/konservatif)
        open_positions = db_open
        logger.debug(f"🔴 RESTORE INCOMPLETE: managed={managed_open} db={db_open} → pakai db")
    else:
        # Normal: weighted ke managed, tapi di-cap oleh DB
        open_positions = min(max(managed_open, int(db_open * 0.6)), db_open) if db_open > 0 else managed_open

    if open_positions > 3:
        exposure_penalty = min(20, open_positions * 2)  # Max +20
        base += exposure_penalty
        logger.debug(f"🔴 EXPOSURE PENALTY: {open_positions} open (managed={managed_open}, db={db_open}) → +{exposure_penalty} to threshold")
...
def register_shadow(coin: str, direction: str, mark: float,
    ...
    optional_fields = {
        "hidden_liquidity": hl.get("score", 0) if isinstance(hl, dict) else 0,
        "micro_acceptance": micro_acc.get("score") if isinstance(micro_acc, dict) else None,
        "failed_risk": failed_risk.get("risk", 1.0) if isinstance(failed_risk, dict) else 1.0,
...
def compute_decision_vector(coin: str, event: TradeEvent, score_long: int, score_short: int,
    ...
    contradiction = (score_long > 55 and score_short > 55)
    contra_penalty = 20 if contradiction else 0      # FIX: 40→20 (terlalu agresif sebelumnya)
...
def execute_decision(coin: str, thesis_data: Dict, confidence_data: Dict,
    ...
    if confidence_data["final_score"] < final_threshold:
        ...
        optional_fields = {
            "hidden_liquidity": hl.get("score", 0) if "hl" in locals() and isinstance(hl, dict) else None,
            "micro_acceptance": micro_acc.get("score") if "micro_acc" in locals() and isinstance(micro_acc, dict) else None,
            "failed_risk": failed_risk.get("risk", 1.0) if "failed_risk" in locals() and isinstance(failed_risk, dict) else None,
            "intent_drift": locals().get("intent_drift"),
            "surprise": locals().get("surprise"),
        }
        for k, v in optional_fields.items():
            if v is not None:
                journal_kwargs[k] = v
    ...
    optional_fields = {
        "hidden_liquidity": hl.get("score", 0) if isinstance(hl, dict) else None,
        "micro_acceptance": micro_acc.get("score") if isinstance(micro_acc, dict) else None,
        "failed_risk": failed_risk.get("risk", 1.0) if isinstance(failed_risk, dict) else None,
        "intent_drift": intent_drift if "intent_drift" in locals() else None,
        "surprise": surprise if "surprise" in locals() else None,
