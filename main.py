...
_SECTOR_MAP = {
    ...
    "SUI": "LAYER1", "APT": "LAYER1", "SEI": "LAYER1",
    # SOL ecosystem (lebih spesifik dari LAYER1 generik)
    "JUP": "SOL_ECO", "PYTH": "SOL_ECO", "RAY": "SOL_ECO",
    # L2 spesifik
    "STRK": "L2", "MANTA": "L2",
    # DeFi
    "UNI": "DEFI", "AAVE": "DEFI", "CRV": "DEFI", "LDO": "DEFI", "GMX": "DEFI",
    "MKR": "DEFI", "LIDO": "DEFI", "DYDX": "DEFI", "SNX": "DEFI",
    # AI/ML
    "RENDER": "AI", "AGIX": "AI", "ARKM": "AI", "OCEAN": "AI", "FET": "AI",
    "TAO": "AI", "RNDR": "AI", "AKT": "AI", "WLD": "AI", "GRT": "AI",
    # Memecoin
...
def build_candidate_pool_v11_final(max_candidates: int = 12) -> List[str]:
    ...
    try:
        ...
        for coin in list(snapshot.mids.keys()):
            ...
            # WARMUP BYPASS: gate_score=0.0 bisa berarti dua hal yang beda --
            # (a) coin memang flat/tidak menarik, ATAU
            # (b) _oi_history/_price_history_1h belum cukup data (oi_growth=0.0
            #     dari get_oi_roc karena len<2, dislocation=0.0 dari
            #     get_dislocation_score_v11 karena price_1h_ago belum ada).
            # Tanpa bypass ini, gate_score<0.5 SELALU true di awal hidup bot
            # untuk SEMUA coin -- gate jadi matematis mustahil dilewati, bukan
            # cuma "ketat". dis_confidence<0.5 adalah sinyal langsung dari
            # get_dislocation_score_v11 bahwa datanya memang belum matang.
            is_warmup_data = (oi_growth == 0.0 and dislocation == 0.0 and dis_confidence < 0.5)
            if gate_score < 0.5 and not is_warmup_data:
                logger.debug(f"  GATE SKIP {coin}: oi={oi_usd/1e6:.1f}M growth={oi_growth:.2f} dis={dislocation:.2f} gate_score={gate_score:.2f}")
                continue
            ...
            if pattern == "LATE":
                continue
            if pattern == "NEUTRAL" and oi_growth < 1.0:
                continue
...
def process_candidates_deep(candidates: List[str], snapshot: MarketSnapshot) -> Tuple[List[dict], int]:
    ...
    FIX dari versi sebelumnya: kalau API budget habis, dulu langsung `break`
    total — kandidat sisanya gak pernah dicoba sama sekali walau sliding
    window budget terus bergeser tiap detik (bisa udah pulih beberapa detik
    kemudian). Sekarang wait sebentar dan retry sekali sebelum skip beneran."""
    results = []
    scan_count = 0

    for coin in candidates:
        # API budget -> WAIT & retry, bukan langsung break (window sliding 60s,
        # tunggu sampai slot tertua benar2 expired, bukan tebakan flat 0.5s
        # yang nyaris gak menggeser window 60 detik)
        if get_api_used() >= API_BUDGET_PER_CYCLE:
            wait_time = get_seconds_until_budget_frees()
            logger.debug(f"⏸️ V11: API budget exhausted, waiting {wait_time:.1f}s before next coin")
        ...
        if not can_call_api():  # global 429-brake tetap dihormati
            ...
            logger.debug(f"⏳ V11: Global API cooldown active, waiting {wait_time:.1f}s")
            time.sleep(wait_time)
            if not can_call_api():
                continue
    ...
    logger.info(f"📊 V11 Deep analysis: {len(results)} alerts from {scan_count} coins scanned, api_used={get_api_used()}")
