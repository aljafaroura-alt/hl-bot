...
# ============================================================
# FORWARD DECLARATION — TradeEvent
# ============================================================
# FIX NameError: TradeEvent dipakai sebagai type hint (unquoted) di Context
# Engine block (contextualize_event_score_v2, contextualize_events_v2,
# select_best_cluster_for_thesis — ditambahkan saat implementasi
# MarketStateVector) yang posisinya di awal file, JAUH sebelum definisi
# dataclass asli TradeEvent (dulu di dekat ContextSnapshot/EventRisk/dll).
# Python evaluasi type hint unquoted secara eager saat def dieksekusi, jadi
# tanpa TradeEvent didefinisikan duluan, module gagal load sama sekali
# (NameError saat import, bukan saat function dipanggil).
#
# HANYA TradeEvent yang dipindah ke sini — dataclass lain (ContextSnapshot,
# EventRisk, MarketReaction, IntentMemory, ZoneQuality, Thesis, PendingSetup,
# MarketSnapshot) TETAP di posisi asal masing-masing karena tidak ada yang
# dipakai sebagai unquoted type hint sebelum definisinya (sudah diverifikasi
# satu per satu). Memindah seluruh blok dataclass berisiko tinggi tanpa
# manfaat tambahan — cukup pindah yang benar-benar menyebabkan crash.
@dataclass
class TradeEvent:
    type: str
    price_low: float
...
CONTEXT_ENGINE_ENABLED = True   # ACTIVATED — Context Engine sekarang mempengaruhi .strength/.confidence event secara live

# MARKET_DNA_ENABLED / DETECTOR_STATS_ENABLED — HANYA guard command Telegram
# (/marketdna, /detectorstats). TIDAK mematikan computation: state_vector
# dipakai Context Engine (contextualize_events_v2) yang jalan tiap cycle
# selama CONTEXT_ENGINE_ENABLED=True, jadi compute_market_state_vector()
# dan update_detector_distribution() TETAP selalu jalan apa pun nilai flag
# ini. Kalau di-False-kan, command-nya cuma bilang "disabled", data di
# baliknya tetap ke-collect normal — supaya matiin display gak diam-diam
# ngerusak Context Engine yang lagi aktif.
MARKET_DNA_ENABLED = True
DETECTOR_STATS_ENABLED = True
...
STRUCTURE_ENGINE = "v2"       # "v1" | "v2" — ACTIVATED setelah observasi Phase B/C selesai
STRUCTURE_COMPARE = False     # Observation logging OFF — V2 sudah jadi engine utama, tidak perlu lagi dibandingkan tiap cycle
...
def cmd_marketdna(m):
    ...
    if m.from_user.id != USER_ID:
        return
    if not MARKET_DNA_ENABLED:
        bot.reply_to(m, "⚪ /marketdna disabled (MARKET_DNA_ENABLED=False). Computation tetap jalan di background untuk Context Engine, cuma display-nya dimatikan.")
        return
...
def cmd_detectorstats(m):
    ...
    if m.from_user.id != USER_ID:
        return
    if not DETECTOR_STATS_ENABLED:
        bot.reply_to(m, "⚪ /detectorstats disabled (DETECTOR_STATS_ENABLED=False). Distribution tetap ke-collect di background.")
        return
