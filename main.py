...
def execute_decision(coin: str, thesis_data: Dict, confidence_data: Dict,
    ...
    with TRADE_MANAGER._lock:
        ...
        coin_positions = [p for p in positions if p.coin == coin and p.status == "OPEN"]
        total_positions = len([p for p in positions if p.status == "OPEN"])
        
        # If BTC exposure > 30%, reduce size for other coins
        ...
        if btc_exposure > 30 and coin != "BTC":
            ...
            logger.info(f"EXPOSURE_DIVERSIFIER {coin}: BTC exposure {btc_exposure:.0f}% > 30% → size * {size_mult_penalty}")
