...
def compute_suggested_leverage(coin: str, entry: float, sl: float, position_size_mult: float) -> Dict[str, Any]:
    ...
    Tier berdasarkan native_cap exchange (dari Hyperliquid meta):
      BTC/ETH (cap >=40x) → target 80% cap  (mis. BTC cap40 → 32x)
      Coin mid  (cap 10-39x) → target 75% cap  (mis. cap10 → 7.5x)
      Coin kecil (cap <10x)  → target 70% cap  (mis. cap5  → 3.5x)

    position_size_mult dari conviction/regime dipakai sebagai fine-tuning
    ±10% di atas target — bukan penentu utama seperti sebelumnya.
    Hasil final selalu dijepit ke native_cap.
    """
    ...
    # Tier ratio berdasarkan native_cap
    if native_cap >= 40:
        base_ratio = 0.80   # BTC, ETH → ~32x dari cap40
    elif native_cap >= 10:
        base_ratio = 0.75   # mid caps → 7-22x
    else:
        base_ratio = 0.70   # small caps → ~3-6x

    # Fine-tune ±10% berdasarkan conviction/regime (position_size_mult 0.5-2.0)
    # Normalize ke range 0.9-1.1 supaya pengaruhnya minor
    size_adj = 0.9 + (min(2.0, max(0.5, position_size_mult)) - 0.5) / 15.0
    raw_suggested = native_cap * base_ratio * size_adj
