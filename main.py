...
def cmd_analytics(m):
    ...
    # 1. Total semua sinyal (termasuk open)
    c.execute("SELECT COUNT(*) FROM signals")
    total_all = c.fetchone()[0] or 0
    
    # 2. Closed (evaluated=1)
    ...
    c.execute('''SELECT 
                ...
                SUM(CASE WHEN outcome IS NULL OR outcome = '' THEN 1 ELSE 0 END) as unknown,
                AVG(rr) as avg_rr,
    ...
    # BE = sisanya (jika outcome ga ada di win/loss)
    be = total_closed - wins - losses - unknown
    if be < 0: be = 0
    
    win_rate = (wins / total_closed * 100) if total_closed > 0 else 0
    expectancy = avg_pnl  # Rata-rata per trade closed
    
    # 5. Signal Efficiency (dari pipeline)
    pipe = get_pipeline_metrics()
    confidence = pipe.get('confidence', 0)
    executed = pipe.get('execute_pass', 0)
    efficiency = (executed / confidence * 100) if confidence > 0 else 0
...
━━━━━━━━━━━━━━━━━━━━━━
📊 <b>Trade Status</b>
├─ Total Signals: {total_all}
├─ Closed: {total_closed}
├─ Open: {total_open}
└─ Shadow (near-miss): {shadow_total}
...
⚙️ <b>Funnel Efficiency</b>
├─ Confidence Pass: {confidence}
├─ Executed: {executed}
└─ Signal Efficiency: {efficiency:.1f}%

💡 Interpretasi:
"""
    eff_emoji, eff_label = get_efficiency_interpretation(executed, confidence)
    text += f"{eff_emoji} {eff_label}\n"

    # Score Buckets (closed trades)
    c.execute('''SELECT 
                    CASE 
...
def cmd_analytics_open(m):
    ...
    conn = db_connect()
    c = conn.cursor()
    c.execute('''SELECT coin, direction, entry_price, sl_price, tp_price, rr, score
    ...
    for coin, direction, entry, sl, tp, rr, score in rows:
