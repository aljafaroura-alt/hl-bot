...
def execute_decision(coin: str, thesis_data: Dict, confidence_data: Dict,
    ...
    # ===== THRESHOLD CHECK + UNIVERSAL JOURNAL =====
    decision_type = "EXECUTE"
    why_not_final = why_not

    if confidence_data["final_score"] < final_threshold:
        decision_type = "REJECT"
        why_not_final = f"score_{confidence_data['final_score']}_lt_{final_threshold}"
        reason_reject = f"score {confidence_data['final_score']:.0f} < {final_threshold}"
...
🚫 *Top Rejections*
"""
    for reason, count in top_reject:
        text += f"├─ {reason}: {count}\n"
    if not top_reject:
        text += "├─ (none)\n"
