...
def get_oi_acceleration(coin: str, window: int = 5) -> float:
    """OI acceleration = linear slope dari window titik OI terakhir.
    POSITIF = OI naik (bullish interest), NEGATIF = OI turun."""
    with _oi_lock:
    ...
    sum_xx = sum(times[i] * times[i] for i in range(n))
    denom = n * sum_xx - sum_x * sum_x
    if denom == 0:
        return 0.0
    slope = (n * sum_xy - sum_x * sum_y) / denom
    return max(-1.0, min(1.0, slope / 10000))
...
def get_spread_compression(coin: str) -> float:
    """Spread compression = seberapa kecil range terkini vs ATR.
    1.0 = sangat compressed (pre-breakout), 0.0 = range besar."""
    candles = get_candles(coin, "5m", 10)
    if not candles or len(candles) < 5:
        return 0.5
    ranges = [float(c['h']) - float(c['l']) for c in candles[-3:]]
    avg_range = sum(ranges) / len(ranges)
    avg_price = float(candles[-1]['c'])
    range_pct = avg_range / avg_price * 100 if avg_price > 0 else 1.0
    atr = get_atr_pct(coin, 14, "1h")
    if atr <= 0:
        atr = 1.0
    return max(0.0, min(1.0, 1.0 - range_pct / atr))
...
def get_best_imbalance_direction(imbalance: Dict[str, Any]) -> Optional[str]:
    """LONG kalau strength > 0.15, SHORT kalau strength < -0.15."""
    if not is_imbalance_valid(imbalance):
        return None
...
def build_candidate_pool_v12(max_candidates: int = 12) -> List[str]:
    ...
    try:
        ...
        for coin in list(snapshot.mids.keys()):
            ...
            if not is_imbalance_valid(imbalance, min_magnitude=0.10):
                continue

            direction = get_best_imbalance_direction(imbalance)
            if not direction:
                continue
        ...
        if not scores:
            logger.warning("Discovery V12: no coins passed imbalance gate, fallback to V11")
        ...
        logger.info(f"🔍 DISCOVERY V12: {len(candidates)} candidates")
        for coin, score in sorted_coins[:8]:
            imp = compute_imbalance_strength(coin)
...
def state_engine_update_v12():
    ...
    # ===== STAGE A: DISCOVERY V12 =====
    candidates = build_candidate_pool_v12(max_candidates=12)
    if not candidates:
...
def scheduled_state_engine_v12():
    ...
    while RUNTIME.is_running():
        ...
        try:
            ...
            logger.info(f"State engine V12 cycle done, next in {interval}s")
        except Exception:
            logger.exception("STATE_ENGINE_CRASH — cycle skipped, thread alive")
        RUNTIME.wait(interval)
