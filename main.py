    ...
    try:
        ...
        c.execute("""
            SELECT outcome, pnl, entry_price, sl_price, tp_price, direction,
                   mfe, mae, exit_price, exit_reason
        ...
        stored_outcome, stored_pnl, entry, sl, tp, direction, mfe, mae, exit_price, exit_reason = row
...
def get_fast_fail_breakdown(limit: int = 100) -> Dict[str, Any]:
    """Breakdown FAST_FAIL trades by score, regime, micro, rr, duration, reason."""
    try:
        conn = db_connect()
        c = conn.cursor()
        c.execute("""
            SELECT 
                score, regime, micro_acceptance, rr, 
                (exit_time - timestamp) / 60.0 as duration_minutes,
                outcome, reason, exit_reason
            FROM signals 
            WHERE evaluated=1 
        ...
        micros = [r[2] for r in rows if r[2] is not None]
        rrs = [r[3] for r in rows if r[3] is not None]
        durations = [r[4] for r in rows if r[4] is not None]
        outcomes = [r[5] for r in rows if r[5]]
        reasons = [r[7] for r in rows if r[7]]
        ...
        return {
            ...
            "avg_micro": round(sum(micros) / len(micros), 1) if micros else 0,
...
"""
    ...
    try:
        ...
        if ff["n"] > 0:
            ...
            text_w11w12 += f"├─ Avg Micro: {ff['avg_micro']:.0f}\n"
