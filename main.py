...
class TradeManager:
    ...
    def _update_profit_lock(self, pos: OpenPosition, current_price: float):
        ...
        if pos.direction == "LONG":
            ...
            if new_sl > pos.sl:
                pos.sl = new_sl
                logger.info(f"🔒 PROFIT LOCK {pos.coin}: SL → {new_sl:.4f} (lock_pct={lock_pct:.3f}%, mfe={mfe_pct:.2f}%, fraction={lock_fraction:.2f})")
        else:
...
def compute_suggested_leverage(coin: str, entry: float, sl: float, position_size_mult: float) -> Dict[str, Any]:
    ...
    tier_suggested = native_cap * base_ratio * size_adj
