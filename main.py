...
def init_websocket():
    ...
    global _ws_manager, _ws_mid, _ws_ob, _ws_healthy
    ...
    with _ws_lock:
        ...
        try:
            ...
            # Subscribe L2 buat top coin by OI (kalau snapshot udah ada)
            try:
                snap = CACHE.get("snapshot")
                if snap and snap.oi:
                    top = sorted(snap.oi.items(), key=lambda x: x[1], reverse=True)[:8]
                    _ws_ob.subscribe_coins([c for c, _ in top if c in snap.mids])
            except Exception as e:
                logger.debug(f"L2 subscribe error: {e}")
...
def cmd_ws(m):
    ...
    text = (
        ...
        f"Candles: REST only (WS candle builder belum diaktifkan)\n"
    )
    bot.reply_to(m, text, parse_mode='HTML')
