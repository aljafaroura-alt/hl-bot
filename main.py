...
TUNABLE = {
    ...
    "ENABLE_STALE_CLEANUP": False,   # aktifkan setelah audit berulang kali dan yakin
...
def log_velocity_trace(
    ...
    """Log per-decision decision-level metrics."""
    try:
        pipe = get_pipeline_metrics()
        logger.info(
            "VELOCITY_TRACE "
...
def execute_decision(coin: str, thesis_data: Dict, confidence_data: Dict,
    ...
    if total_open >= MAX_TOTAL_OPEN:
        ...
        log_skip(coin=coin, reason="inventory_full", stage="GATE", cache_age=cache_age)
        return None
    ...
    if coin_open >= MAX_COIN_OPEN:
        ...
        log_skip(coin=coin, reason="coin_limit", stage="GATE", cache_age=cache_age)
        return None
    ...
    if clarity.get("dominant_factor") == "strong_contradiction":
        ...
        update_fatigue_memory(event.type)
        log_skip(coin=coin, reason="strong_contradiction", stage="GATE", cache_age=cache_age)
        return None
    ...
    if clarity["decision_quality"] < 0.40:  # Only skip on extreme chaos
        ...
        update_fatigue_memory(event.type)
        log_skip(coin=coin, reason=f"chaos_{clarity['decision_quality']:.2f}", stage="GATE", cache_age=cache_age)
        return None
        ...
        with TRADE_MANAGER._lock:
            ...
            if total_open > 0:
                ...
                if btc_share > 0.60 and coin != "BTC":
                    ...
                    update_fatigue_memory(event.type)
                    log_skip(coin=coin, reason="btc_concentration", stage="GATE", cache_age=cache_age)
                    return None
    ...
    if not conviction_data["is_qualified"]:
        ...
        update_fatigue_memory(event.type)
        log_skip(coin=coin, reason=reason, stage="GATE", cache_age=cache_age)
        return None
    ...
    if not micro_confirmed:
        ...
        update_fatigue_memory(event.type)
        log_skip(coin=coin, reason="micro_structure_gate", stage="GATE", cache_age=cache_age)
        return None
    ...
    if is_entry_cooldown_active(coin, event.direction):
        ...
        log_skip(coin=coin, reason="cooldown_active", stage="GATE", cache_age=cache_age, data_source=data_source)
        return None  # BATAL
    ...
    if is_position_open(coin, event.direction):
        ...
        inc_pipeline_counter("reject_duplicate")
        log_skip(coin=coin, reason="position_already_open", stage="GATE", cache_age=cache_age, data_source=data_source)
        return None  # BATAL
...
🛠️ <b>MAINTENANCE</b>
/staleaudit → Check stale open positions (>24h)
...
def cmd_cachehealth(m):
    ...
    text = (
        ...
        f"💡 <b>Interpretasi</b>\n"
        f"{'✅ EXEC cache <30s — healthy' if avg_exec < 30 else f'🟡 EXEC cache {avg_exec:.0f}s — pertimbangin tuning'}\n"
        f"{'✅ Thesis cache <180s — ok' if avg_thesis < 180 else f'🟡 Thesis cache {avg_thesis:.0f}s — ok for structure'}\n"
        f"{'✅ Hit rate >60% — cache working' if hit_rate > 60 else f'🟡 Hit rate {hit_rate:.0f}% — room for improvement'}\n"
        f"{'✅ UNKNOWN=0 — source lengkap' if sources.get('UNKNOWN', 0) == 0 else '🔴 UNKNOWN >0 — cek metadata injection'}"
    )
    bot.reply_to(m, text, parse_mode='HTML')
...
def cmd_velocity(m):
    ...
    for line in lines:
        parts = line.split()
        try:
            score = float(next(p for p in parts if p.startswith("score=")).split("=")[1])
            threshold = float(next(p for p in parts if p.startswith("threshold=")).split("=")[1])
            gap = float(next(p for p in parts if p.startswith("gap=")).split("=")[1])
    ...
    score_note = "🔴 Score floor? Median &lt; 68 — mungkin stuck." if median_score < 68 else "✅ Score sehat."
    gap_note = "🟡 Threshold terlalu tinggi? Median gap &lt; -3." if median_gap < -3 else "✅ Threshold balanced."
    gate_note = "🟢 Position gate aktif (PASS tinggi)" if pos_gates.get("PASS", 0) > pos_gates.get("CLEAR", 0) * 0.3 else "✅ Position gate bersih."

    text = (
        f"📊 <b>VELOCITY</b> (last {len(scores)} decisions)\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📈 <b>Score</b>\n"
...
"""
    ...
    if result['distribution']['be'] > result['total'] * 0.5 if result['total'] > 0 else False:
        text += f"\n🔴 <b>WARNING</b>: BE > 50% — possible exit price issue!"
...
"""
    ...
    try:
        sig_health = check_signal_db_health()
        if sig_health:
...
├─ Orphan (stale &gt;6h, untracked): {orphan_count}
...
def cmd_staleaudit(m):
    ...
    now = time.time()
    lines = ["📊 STALE AUDIT (age > 24h, still OPEN)\n━━━━━━━━━━━━━━━━━━━━━━"]
    with TRADE_MANAGER._lock:
    ...
    if total_stale == 0:
        bot.reply_to(m, "✅ No stale trades (>24h) found.")
        return
