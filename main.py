...
def compute_dynamic_magnitude_gate(regime: str = "RANGING") -> float:
    """
    Gate ngikut distribusi market, bukan angka statis 0.10.

    RANGING:          p70 (sinyal subtle, gate di persentil-70)
    TRENDING_UP/DOWN: antara p70-p90 (sinyal lebih jelas, gate lebih tinggi)
    CHAOS/PANIC:      antara p70-p90 tapi lebih rendah dari trending (butuh
                       sinyal lebih kuat buat filter noise, tapi jangan
                       terlalu strict)
    Cold start (n<20): fallback ke default lama 0.10 (SAMA seperti sebelumnya,
                        jadi behavior gak berubah sampai distribution matang).

    Clamp aman: [0.03, 0.15] — gak akan pernah lebih permisif dari
    sepertiga gate lama, dan gak akan pernah lebih strict dari 1.5x gate lama.
    """
    dist = get_magnitude_distribution()

    if dist.get("n", 0) < 20:
        return 0.10  # cold start: identik dengan behavior lama
    ...
    gate = max(0.03, min(0.15, gate))

    logger.info(f"MAG_GATE regime={regime} p70={p70:.4f} p90={p90:.4f} gate={gate:.4f}")
...
def execute_decision(coin: str, thesis_data: Dict, confidence_data: Dict,
    ...
    leverage_info = compute_suggested_leverage(
        ...
        market_regime=thesis_data.get("market_regime", "UNKNOWN"),
        confidence_data=confidence_data,  # ← L2: untuk Entry Quality
    )
...
def get_progressive_gate(coin: str, base_magnitude: float = 0.10) -> float:
    ...
    Range: 0.07 (kondisi sangat jelas) – 0.10 (noisy/default).
    Manggil interpret_regime_v10 (ada candle cost) tapi HANYA untuk coin
    yang sudah lolos is_imbalance_valid(gate=0.10), bukan semua ~230 coin.
    """
    try:
        regime_int = interpret_regime_v10(coin)
        ...
        return max(0.07, min(base_magnitude, effective))
    except Exception:
...
def get_context_set(snapshot: MarketSnapshot, top_n: int = 3) -> List[str]:
    ...
    """
    if not snapshot or not snapshot.mids:
        return get_fallback_candidates(top_n)
    ...
    try:
        ...
        for coin in snapshot.mids.keys():
            ...
            # OI 60% (modal komit) + Volume 40% (aktivitas) — coin alt
            # dengan OI/volume dominan bisa ngalahin BTC/ETH kalau
            # emang lagi jadi pusat perhatian modal.
        ...
        logger.debug(f"get_context_set error (non-fatal): {e}")
        return get_fallback_candidates(top_n)


def get_regime_from_context(context_coins: List[str]) -> str:
    """Baca regime market dari context coins (majority vote). Ini CUMA
    buat info/logging/observability — TIDAK dipakai buat filter atau
    gate candidate pool, jadi gak ada risiko displace alt coin."""
...
def build_candidate_pool_v12(max_candidates: int = 12) -> List[str]:
    ...
    try:
        ...
        if not scores and imbalance_cache:
            _relaxed_gate = max(0.03, _dynamic_base_gate * 0.7)
            logger.warning(
                f"[discovery] pass1 empty (gate={_dynamic_base_gate:.4f}), "
                f"retry relaxed gate={_relaxed_gate:.4f} on {len(imbalance_cache)} cached coins"
            )
...
def state_engine_update_v12():
    ...
    if snap:
        ...
        # get_context_set() milih coin dinamis by OI+Volume (bukan
        # hardcode BTC/ETH/SOL), TAPI hasilnya cuma dipake buat baca
        # regime market — TIDAK filter/displace candidate pool.
        try:
            _ctx_coins = get_context_set(snap, top_n=3)
            _ctx_regime = get_regime_from_context(_ctx_coins)
            logger.info(f"📊 REGIME from context {_ctx_coins}: {_ctx_regime}")
        except Exception as e:
    ...
    # ===== STAGE A: DISCOVERY V12 (rescue-aware + phase-aware) =====
    if mode == "RESCUE":
