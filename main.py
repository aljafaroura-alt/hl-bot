...
def get_oi_roc(coin: str) -> float:
    """Rate of change of OI (5min window) [DEBUG]"""
    with _oi_lock:
        if coin not in _oi_history or len(_oi_history[coin]) < 2:
            hist_len = len(_oi_history.get(coin, deque())) if coin in _oi_history else 0
            logger.debug(f"ROC_{coin} NO_HISTORY len={hist_len}")
            return 0.0
        ...
        oi_vals = [v for ts, v in hist if ts >= cutoff]
        
        if not oi_vals:
            logger.debug(f"ROC_{coin} NO_RECENT 5min_empty")
            return 0.0
        ...
        if oi_avg == 0:
            logger.debug(f"ROC_{coin} AVG_ZERO")
            return 0.0
        
        roc = (oi_current - oi_avg) / oi_avg * 100
        
        logger.debug(f"ROC_{coin} n={len(oi_vals)} avg={oi_avg:.0f} cur={oi_current:.0f} roc={roc:+.1f}%")
...
""")
    ...
    for i, coin in enumerate(candidates):
        ...
        # Check 4: Endpoint available
        endpoint_ok = can_call_api_endpoint("candles")
        logger.info(f"   endpoint={endpoint_ok}")
        if not endpoint_ok:
            logger.warning(f"   SKIP_{coin}_no_endpoint")
            continue
