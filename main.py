...
def cmd_health(m):
    ...
    with _journal_lock:
        journal_size = len(_decision_journal)
        if journal_size > 0:
            last_entry = _decision_journal[-1]
            last_decision_age = int(time.time() - last_entry.timestamp)
            last_decision_str = f"{last_decision_age // 60}m {last_decision_age % 60}s ago"
        else:
            last_decision_str = "N/A"
    if journal_size > 0:
    ...
    # ===== DCR DISPLAY =====
    dcr = pipe.get("dcr")
    is_meaningful = pipe.get("is_meaningful", False)
    uptime_m = pipe.get("uptime_minutes", 0)

    if not is_meaningful:
        dcr_display = f"⏳ Warming ({uptime_m:.0f}m, {pipe.get('check', 0)} scans)"
    elif dcr is None:
        dcr_display = "N/A"
    else:
        dcr_display = f"{dcr}%"
        if dcr < 0.3:
            issues.append("DCR too strict")
        elif dcr > 5.0:
            issues.append("DCR too loose")
...
├─ Execute Pass: {pipe.get('execute_pass', 0)}
├─ Journal: {journal_size} ({last_decision_str})
├─ Rejections: {pipe.get('total_reject', 0)}
├─ DCR: {dcr_display}
└─ Funnel: {pipe.get('funnel_issue', '✅ FUNNEL HEALTHY')}
