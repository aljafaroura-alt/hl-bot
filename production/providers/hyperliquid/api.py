...
class HyperliquidAPI:
    ...
    def _is_tradable(self, coin: str) -> bool:
        """
        Cek FRESH (dengan 1x force-refresh kalau perlu) apakah coin ini
        beneran ada di universe exchange. Dipakai SEBELUM place order,
        supaya kalau coin gak ketemu (delisted / belum listing di testnet /
        typo nama), kita return error yang JELAS ("coin_not_in_universe_X")
        daripada biarin SDK hyperliquid-python-sdk lempar KeyError mentah
        dari internal coin_to_asset dict-nya sendiri — yang keliatan di log
        cuma sebagai pesan error kosong berupa nama coin doang (misal
        "'ENA'"), sama sekali gak jelas itu KeyError apa dan kenapa.
        """
        meta = self._get_meta()
        names = {a.get("name") for a in meta.get("universe", [])}
        if coin in names:
            return True
        # Force-refresh sekali — mungkin coin baru listing setelah cache
        # pertama kita fetch pas startup.
    ...
    def place_market_order(
        ...
        if not self._is_tradable(coin):
            logger.error(
                f"❌ {coin} TIDAK ADA di universe exchange (mode={self._mode}) — "
                f"coin belum/tidak listing di sini (beda dari mainnet), delisted, "
                f"atau nama salah (cek prefix kayak 'k' buat kBONK dsb). Order "
                f"DIBATALKAN sebelum sempat kena KeyError mentah dari SDK."
            )
            return {"status": "err", "error": f"coin_not_in_universe_{coin}"}
    ...
    def place_limit_order(
        ...
        if not self._is_tradable(coin):
            logger.error(
                f"❌ {coin} TIDAK ADA di universe exchange (mode={self._mode}) — "
                f"coin belum/tidak listing di sini (beda dari mainnet), delisted, "
                f"atau nama salah. Order DIBATALKAN sebelum sempat kena KeyError "
                f"mentah dari SDK."
            )
            return {"status": "err", "error": f"coin_not_in_universe_{coin}"}
        size = self._round_size(coin, size)
