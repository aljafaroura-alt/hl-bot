...
class HyperliquidOrders:
    ...
    def update_stop_loss(self, coin: str, old_order_id, new_price: float, size: float) -> ExecutionResult:
        """Trailing SL beneran: cancel SL lama (best-effort) + pasang SL baru."""
        try:
            if old_order_id:
                try:
                    old_coin, old_oid = self._decode_order_id(old_order_id)
                    cancel_resp = self._api.cancel_order(old_coin, old_oid)
                    if cancel_resp.get("status") != "ok":
                        # Best-effort: order lama mungkin udah filled/gak ada lagi.
                        # Ini bukan alasan buat gagalin trailing SL yang baru.
                        logger.warning(f"cancel old SL {old_order_id} gak sukses (mungkin udah gak ada): {cancel_resp}")
                except Exception as ce:
                    logger.warning(f"cancel old SL {old_order_id} error (lanjut pasang SL baru): {ce}")
