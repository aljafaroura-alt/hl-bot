...
class HyperliquidPositions:
    ...
    def is_open(self, coin: str) -> bool:
        """
        Cek FRESH (bukan cache) apakah posisi coin ini beneran masih terbuka
        di exchange sekarang. Query langsung ke user_state(), bukan pakai
        state internal manapun (Brain atau Engine) — dibuat khusus buat
        keperluan verifikasi/cross-check sebelum notifikasi status posisi
        dikirim, supaya asumsi internal (misal "trailing_sl hit" versi
        Brain) bisa dikonfirmasi dulu ke kenyataan di exchange.

        Return True kalau posisi coin ini punya size != 0 di exchange,
        False kalau ga ketemu / size 0 / gagal fetch (fail-safe: kalau ragu
        karena error network, mending dianggap "belum tentu closed" —
        caller yang butuh sikap konservatif bisa treat False sebagai
        'tidak terkonfirmasi', bukan otomatis 'sudah pasti closed').
        """
        try:
            state = self._api.user_state()
            for ap in state.get("assetPositions", []):
            ...
            logger.error(f"❌ is_open({coin}) failed: {e}")
            return False
