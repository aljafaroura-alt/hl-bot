...
class ProductionEngine:
    ...
    def execute(self, decision: Decision) -> ExecutionResult:
        ...
        # === STEP 4.5: Set leverage (best-effort, non-blocking) ===
        ...
        # (misal 2.6x -> 3x), bukan default lama yang nempel di exchange.
        # PaperProvider gak punya method ini -> otomatis skip (getattr None).
        set_lev_fn = getattr(self._provider, "set_leverage", None)
        if set_lev_fn:
            try:
                set_lev_fn(decision.coin, decision.leverage)
            except Exception as e:
                logger.warning(f"set_leverage gagal, lanjut pakai leverage default exchange: {e}")
        ...
        self._open_notional[decision.signal_id] = {
            ...
            "sl_order_id": None,
            "tp_order_id": None,
            # tp_price disimpan biar trailing tick bisa re-arm TP di harga yang
            # sama tiap kali sweep-all nyapu order lama (lihat update_stop_loss
            # di bawah) — sebelum ini TP price gak pernah ditrack, jadi TP
            # yang ke-cancel gara-gara sweep gak pernah bisa dipasang ulang.
