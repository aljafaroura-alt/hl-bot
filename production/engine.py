...
class MoneyManager:
    ...
    def __init__(self, config: ProductionConfig):
        ...
        # Floor ABSOLUT berbasis % equity (independen dari tier/size_mult).
        # Tier-based floor di atas ini kadang tetap kekecilan buat tier
        # rendah (D/C, size_mult 0.5x-1.5x) -- margin bisa jatuh ke $5-14
        # walau equity ratusan dolar, karena floor lama ikut size_mult tier.
        # Floor equity-based ini jadi JARING PENGAMAN TERAKHIR: berapapun
        # tier-nya (selama size_mult > 0, bukan tier E/SHADOW yang sengaja
        # skip), margin gak boleh jatuh di bawah persentase equity ini.
        # size_mult tetap dipakai buat naikin size DI ATAS floor ini kalau
        # tier-nya memang tinggi -- filosofi "modal terkonsentrasi ke setup
        # terbaik" tetap jalan, cuma sekarang ada dasar minimum yang gak
        # bisa ditembus ke bawah.
        self._min_margin_pct_of_equity = getattr(config, "MIN_MARGIN_PCT_OF_EQUITY", 10.0) / 100.0
    ...
    def calculate_position_size(self, decision: Decision, wallet: Wallet) -> dict:
        ...
        if has_slot_info:
            ...
            # === EQUITY-BASED MARGIN FLOOR (absolut, independen tier) ===
            # Floor tier-based di atas kadang tetap kekecilan buat tier
            # rendah (D/C) -- size_mult 0.5x-1.5x bikin margin jatuh ke
            # recehan walau equity gede. Floor ini jaring pengaman terakhir:
            # margin gak boleh di bawah MIN_MARGIN_PCT_OF_EQUITY% dari
            # equity, KECUALI size_mult == 0 (tier E/SHADOW, sengaja "jangan
            # entry beneran" -- floor ini gak boleh maksa tier itu ikut naik).
            # size_usdc di sini masih NOTIONAL, sedangkan target-nya dihitung
            # di level MARGIN dulu (equity x pct), baru dikonversi balik ke
            # notional pakai leverage -- biar floor-nya beneran ke margin,
            # bukan ke notional (yang bakal salah kalau leverage tinggi).
            if size_mult > 0:
                _lev_for_floor = decision.leverage if decision.leverage and decision.leverage > 0 else 1.0
                target_margin_floor = wallet.usdc_balance * self._min_margin_pct_of_equity
                notional_floor = target_margin_floor * _lev_for_floor
                if size_usdc < notional_floor:
                    logger.info(
                        f"📐 EQUITY MARGIN FLOOR applied: {decision.coin} size_usdc "
                        f"${size_usdc:.2f} -> ${notional_floor:.2f} "
                        f"(target_margin=${target_margin_floor:.2f} = "
                        f"{self._min_margin_pct_of_equity*100:.0f}% equity x lev={_lev_for_floor:.1f}x)"
                    )
                    size_usdc = notional_floor
        ...
        #
        # PENTING: cap ini dihitung di level MARGIN, bukan notional.
        # "Maks 30% modal per trade" cuma make sense diukur dari uang yang
        # BENERAN dipertaruhkan (margin) — dulu di-cap sebagai notional
        # flat, yang berarti di leverage tinggi (5x-10x) cap ini jauh lebih
        # ketat dari yang dimaksud (leverage 10x -> notional 30% equity
        # cuma margin 3% equity), sementara equity-based margin floor di
        # atas butuh margin ~10% equity supaya "makan slot secukupnya
        # sesuai max_slots" bisa kepenuhi di leverage berapapun. Dihitung
        # di margin, cap ini konsisten independen dari leverage.
        _lev_for_exposure_cap = decision.leverage if decision.leverage and decision.leverage > 0 else 1.0
        max_margin_per_trade = wallet.usdc_balance * self._max_exposure
        max_size_usdc = max_margin_per_trade * _lev_for_exposure_cap
