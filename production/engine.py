...
class MoneyManager:
    ...
    def calculate_position_size(self, decision: Decision, wallet: Wallet) -> dict:
        ...
        1. SLOT-BASED (dipakai kalau decision.open_slots_used DAN
           ...
           free_margin / sisa_slot. Adaptif — free_margin naik/turun sesuai
        ...
        # Cap max exposure per trade tetap berlaku di KEDUA mode — safety
        # net biar 1 slot gak bisa dialokasikan lebih dari batas absolut,
        # meskipun hasil hitungan slot-based/risk-based di atas gede.
...
class ProductionEngine:
    ...
    def execute(self, decision: Decision) -> ExecutionResult:
        ...
        if is_open_fn:
            try:
                already_open = is_open_fn(decision.coin)
            except Exception as e:
                logger.error(
                    f"❌ is_position_open exception {decision.coin}: {e} — "
                    f"entry DIBATALKAN (gak bisa pastikan aman dari numpuk leverage)"
                )
                return ExecutionResult(
                    success=False,
                    error=f"is_position_open_exception: {e}",
                )
            if already_open:
                logger.error(
                    f"🚨 ENTRY DIBATALKAN: {decision.coin} SUDAH ADA posisi terbuka "
                    f"di exchange — entry baru akan numpuk (netting) ke posisi lama "
                    f"dan ikut leverage lama itu, BUKAN leverage {decision.leverage:.1f}x "
                    f"yang baru dihitung Brain. Tutup/bersihkan posisi lama dulu sebelum "
                    f"entry baru untuk coin ini."
                )
                return ExecutionResult(
                    success=False,
                    error=f"position_already_open_{decision.coin}_would_merge_leverage",
                )
